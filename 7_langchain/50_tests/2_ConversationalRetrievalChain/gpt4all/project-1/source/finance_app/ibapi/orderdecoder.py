...
class OrderDecoder(Object):
    ...
    def decodeScaleOrderParams(self, fields):
        if self.version >= 20:
            ...
            self.order.notSuppScaleNumComponents = decode(
        ...
        if (
            ...
            self.order.scalePriceAdjustValue = decode(
                float, fields, SHOW_UNSET
            )
            self.order.scalePriceAdjustInterval = decode(
                int, fields, SHOW_UNSET
            )
    ...
    def decodeDeltaNeutral(self, fields):
        if self.version >= 20:
            ...
            if deltaNeutralContractPresent:
                ...
                self.contract.deltaNeutralContract.delta = decode(
                    float, fields
                )
                self.contract.deltaNeutralContract.price = decode(
                    float, fields
                )
    ...
    def decodeSoftDollarTier(self, fields):
        if self.serverVersion >= MIN_SERVER_VER_SOFT_DOLLAR_TIER:
            ...
            self.order.softDollarTier = SoftDollarTier(
