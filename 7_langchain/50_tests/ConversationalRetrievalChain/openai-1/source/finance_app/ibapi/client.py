...
class EClient(object):
    ...
    def setConnState(self, connState):
        ...
        logger.debug(
            "%s connState: %s -> %s" % (id(self), _connState, self.connState)
        )
    ...
    def startApi(self):
        ...
        if not self.isConnected():
            self.wrapper.error(
                NO_VALID_ID, NOT_CONNECTED.code(), NOT_CONNECTED.msg()
            )
            return
    ...
    def connect(self, host, port, clientId):
        ...
        try:
            ...
            logger.debug(
                "ANSWER Version:%d time:%s", server_version, conn_time
            )
    ...
    def run(self):
        ...
        try:
            while not self.done and (
                self.isConnected() or not self.msg_queue.empty()
            ):
                try:
                    try:
    ...
    def reqMktData(
        ...
        if not self.isConnected():
            self.wrapper.error(
                reqId, NOT_CONNECTED.code(), NOT_CONNECTED.msg()
            )
            return
        ...
        if contract.secType == "BAG":
            comboLegsCount = (
                len(contract.comboLegs) if contract.comboLegs else 0
            )
            flds += [
    ...
    def placeOrder(self, orderId: OrderId, contract: Contract, order: Order):
        ...
        if not self.isConnected():
            self.wrapper.error(
                orderId, NOT_CONNECTED.code(), NOT_CONNECTED.msg()
            )
            return
        ...
        VERSION = (
            27 if (self.serverVersion() < MIN_SERVER_VER_NOT_HELD) else 45
        )
        ...
        if self.serverVersion() >= MIN_SERVER_VER_ALGO_ORDERS:
            ...
            if order.algoStrategy:
                algoParamsCount = (
                    len(order.algoParams) if order.algoParams else 0
                )
    ...
    def reqIds(self, numIds: int):
        ...
        msg = (
            make_field(OUT.REQ_IDS) + make_field(VERSION) + make_field(numIds)
        )
